...
(function (factory) {
    ...
    function parseLocalName(token) {
        if (Keyword_1.isKeyword(109, token)) return '_';else if (token instanceof Token_1.NameToken) return token.name;else throw context_1.fail(token.loc, _ => _.expectedLocalName(token));
    }
    ...
    function parseLocalDeclareOrFocus(tokens) {
        if (tokens.isEmpty()) return locals_1.LocalDeclare.focus(tokens.loc);else {
            ...
            if (token instanceof Group_1.GroupSpace) {
                ...
                if (Keyword_1.isKeyword(96, slice.head())) return locals_1.LocalDeclare.typedFocus(tokens.loc, parseSpaced_1.default(slice.tail()));
            }
    ...
    function parseLocalPartsFromSpaced(tokens) {
        ...
        var _ref = Keyword_1.isKeyword(132, tokens.head()) ? [tokens.tail(), 1, false] : orMember && Keyword_1.isKeyword(101, tokens.head()) ? [tokens.tail(), 0, true] : [tokens, 0, false];
