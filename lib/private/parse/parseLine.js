...
(function (factory) {
    ...
    function parseLine(tokens) {
        ...
        if (head instanceof Keyword_1.default) switch (head.kind) {
            case 87:
            case 114:
                return parseAssert(head.kind === 114, rest());
            case 91:
    ...
    const lineSplitKeywords = new Set([88, 89, 134, 139]);
    ...
    function parseMutate(before, after, loc) {
        ...
        if (before.size() === 1) {
            ...
            if (token instanceof Group_1.GroupSpace) {
                ...
                var _Op_1$caseOp = Op_1.caseOp(spaced.opSplitOnce(_ => Keyword_1.isKeyword(97, _)), _ref2 => {
                ...
                if (Keyword_1.isKeyword(102, assignee.nextToLast())) {
    ...
    function parseObjEntry(before, after, loc) {
        if (before.size() === 1) {
            const token = before.head();
            const isName = Keyword_1.isKeyword(136, token);
            const value = () => parseExpr_1.default(after);
    ...
    function parseAssign(localsTokens, after, loc) {
        const value = parseExpr_1.default(after);
