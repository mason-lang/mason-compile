...
(function (factory) {
    ...
    function parseSpaced(tokens) {
        const h = tokens.head(),
              rest = tokens.tail();
        if (h instanceof Keyword_1.KeywordPlain) switch (h.kind) {
            case 34:
    ...
    function parseSpacedFold(start, rest) {
        ...
        for (let i = rest.start; i < rest.end; i = i + 1) {
            ...
            const loc = token.loc;
            if (token instanceof Keyword_1.KeywordPlain) switch (token.kind) {
                case 34:
    ...
    function parseAmpersand(loc, tokens) {
        ...
        var _ref = (() => {
            if (h instanceof Group_1.GroupParenthesis) return [new Fun_1.FunSimple(loc, parseExpr_1.default(Slice_1.Tokens.of(h))), tail];else if (h instanceof Keyword_1.KeywordOperator) return [new Fun_1.FunOperator(loc, h.kind), tail];else if (h instanceof Keyword_1.KeywordUnaryOperator) return [new Fun_1.FunUnary(loc, h.kind), tail];else if (Keyword_1.isKeyword(41, h)) {
                const h2 = tail.head();
                return [new Fun_1.FunGetter(h2.loc, parseMemberName_1.default(h2)), tail.tail()];
            } else return [new Fun_1.FunMember(h.loc, null, parseMemberName_1.default(h)), tail];
        })();
