...
(function (factory) {
    ...
    function parseExcept(tokens) {
        const lines = parseBlock_1.justBlock(106, tokens);
    ...
    function takeTried(lines) {
        ...
        checks_1.checkKeyword(170, line.head());
        return [parseBlock_1.parseJustBlock(170, line.tail()), lines.tail()];
    }
    function takeCatches(lines) {
        ...
        while (!lines.isEmpty()) {
            const line = lines.headSlice();
            if (!Keyword_1.isKeyword(94, line.head())) break;
    ...
    function opTakeElse(lines) {
        ...
        return Keyword_1.isKeyword(105, tokenElse) ? [parseBlock_1.parseJustBlock(105, line.tail()), lines.tail()] : [null, lines];
    }
    function parseOpFinally(lines) {
        if (lines.isEmpty()) return null;
        const line = lines.headSlice();
        checks_1.checkKeyword(109, line.head());
        context_1.check(lines.size() === 1, lines.loc, _ => _.nothingAfterFinally);
        return parseBlock_1.parseJustBlock(109, line.tail());
    }
});
