...
(function (factory) {
    ...
    function parseClassHeader(tokens) {
        var _tokens$getKeywordSec = tokens.getKeywordSections(106, 167);
    ...
    function opTakeConstructor(tokens) {
        ...
        return Keyword_1.isKeyword(97, line.head()) ? [parseConstructor(line.tail()), tokens.tail()] : [null, tokens];
    }
    function parseConstructor(tokens) {
        var _parseFunBlock_1$funA = parseFunBlock_1.funArgsAndBlock(tokens, false, true);

        const args = _parseFunBlock_1$funA.args;
        const memberArgs = _parseFunBlock_1$funA.memberArgs;
        const opRestArg = _parseFunBlock_1$funA.opRestArg;
        const block = _parseFunBlock_1$funA.block;
