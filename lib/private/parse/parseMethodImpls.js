...
(function (factory) {
    ...
    function opTakeDo(lines) {
        const line = lines.headSlice();
        return Keyword_1.isKeyword(100, line.head()) ? [new classTraitCommon_1.ClassTraitDo(line.loc, parseBlock_1.parseJustBlock(100, line.tail())), lines.tail()] : [null, lines];
    }
    exports.opTakeDo = opTakeDo;
    function parseMethodImpl(tokens) {
        var _tokens$takeKeywords = tokens.takeKeywords(136, 175, 156);
        ...
        if (isGetSet(head)) {
            ...
            const ctr = head.kind === 126 ? classTraitCommon_1.MethodGetter : classTraitCommon_1.MethodSetter;
    ...
    function isGetSet(token) {
        if (token instanceof Keyword_1.default) return token.kind === 126 || token.kind === 160;else return null;
    }
