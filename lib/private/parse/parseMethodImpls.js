...
(function (factory) {
    ...
    function takeStatics(lines) {
        if (lines.isEmpty()) return [[], lines];else {
            ...
            return Keyword_1.isKeyword(57, line.head()) ? [parseMethodImpls(parseBlock_1.justBlock(57, line.tail())), lines.tail()] : [[], lines];
        }
    }
    ...
    function opTakeDo(lines) {
        const line = lines.headSlice();
        return Keyword_1.isKeyword(40, line.head()) ? [new classTraitCommon_1.ClassTraitDo(line.loc, parseBlock_1.parseJustBlock(40, line.tail())), lines.tail()] : [null, lines];
    }
    exports.opTakeDo = opTakeDo;
    function parseMethodImpl(tokens) {
        var _tokens$takeKeywords = tokens.takeKeywords(52, 60, 54);
        ...
        if (isGetSet(head)) {
            ...
            const options = _parseMethodSplit_1$d.options;
    ...
    function isGetSet(token) {
        if (token instanceof Keyword_1.KeywordPlain) return token.kind === 47 || token.kind === 56;else return null;
    }
