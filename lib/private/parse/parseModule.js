...
(function (factory) {
    ...
    function parseModule(lines) {
        ...
        var _takeImports = takeImports(123, rest1);
        ...
        var _takeImports3 = takeImports(125, rest2);
    ...
    function parseImports(importKeywordKind, tokens) {
        ...
        return lines.mapSlices(line => {
            ...
            var _parseThingsImported = parseThingsImported(rest, name, importKeywordKind === 125);
    ...
    function takeImportDos(lines) {
        if (!lines.isEmpty()) {
            const line = lines.headSlice();
            if (Keyword_1.isKeyword(124, line.head())) return [parseImportDos(line.tail()), lines.tail()];
        }
        return [[], lines];
    }
    function parseImportDos(tokens) {
        const lines = parseBlock_1.justBlock(124, tokens);
        return lines.mapSlices(line => {
    ...
    function parseThingsImported(tokens, name, isLazy) {
        ...
        if (tokens.isEmpty()) return { imported: [], opImportDefault: importDefault() };else {
            var _ref = Keyword_1.isKeyword(103, tokens.head()) ? [importDefault(), tokens.tail()] : [null, tokens];
        ...
        return Op_1.caseOp(parseName_1.tryParseName(token), name => ({ path: name, name: name }), () => {
            if (token instanceof Group_1.GroupSpace) {
                ...
                while (true) {
                    ...
                    checks_1.checkKeyword(95, rest.head());
                    rest = rest.tail();
                }
