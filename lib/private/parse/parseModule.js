...
(function (factory) {
    ...
    function parseModule(lines) {
        ...
        var _takeImports = takeImports(130, rest1);
        ...
        var _takeImports3 = takeImports(132, rest2);
    ...
    function parseImports(importKeywordKind, tokens) {
        ...
        return lines.mapSlices(line => {
            ...
            var _parseThingsImported = parseThingsImported(rest, name, importKeywordKind === 132);
    ...
    function takeImportDos(lines) {
        if (!lines.isEmpty()) {
            const line = lines.headSlice();
            if (Keyword_1.isKeyword(131, line.head())) return [parseImportDos(line.tail()), lines.tail()];
        }
        return [[], lines];
    }
    function parseImportDos(tokens) {
        const lines = parseBlock_1.justBlock(131, tokens);
        return lines.mapSlices(line => {
    ...
    function parseThingsImported(tokens, name, isLazy) {
        ...
        if (tokens.isEmpty()) return { imported: [], opImportDefault: importDefault() };else {
            var _ref = Keyword_1.isKeyword(110, tokens.head()) ? [importDefault(), tokens.tail()] : [null, tokens];
        ...
        return Op_1.caseOp(parseName_1.tryParseName(token), name => ({ path: name, name: name }), () => {
            if (token instanceof Group_1.GroupSpace) {
                ...
                while (true) {
                    ...
                    checks_1.checkKeyword(102, rest.head());
                    rest = rest.tail();
                }
