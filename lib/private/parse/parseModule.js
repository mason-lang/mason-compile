...
(function (factory) {
    ...
    function parseModule(lines) {
        ...
        var _takeImports = takeImports(48, rest1);
        ...
        var _takeImports3 = takeImports(50, rest2);
    ...
    function parseImports(importKeywordKind, tokens) {
        ...
        return lines.mapSlices(line => {
            ...
            var _parseThingsImported = parseThingsImported(rest, name, importKeywordKind === 50);
    ...
    function takeImportDos(lines) {
        if (!lines.isEmpty()) {
            const line = lines.headSlice();
            if (Keyword_1.isKeyword(49, line.head())) return [parseImportDos(line.tail()), lines.tail()];
        }
        return [[], lines];
    }
    ...
    function parseThingsImported(tokens, name, isLazy) {
        ...
        if (tokens.isEmpty()) return { imported: [], opImportDefault: importDefault() };else {
            var _ref = Keyword_1.isKeyword(46, tokens.head()) ? [importDefault(), tokens.tail()] : [null, tokens];
