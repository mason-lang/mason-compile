(function (factory) {
    if (typeof module === 'object' && typeof module.exports === 'object') {
        ...
        define(["require", "exports", '../ast/locals', '../ast/Val', '../context', '../token/Group', '../token/Keyword', '../token/Token', './checks', './parseBlock', './parseExpr', './parseObjSimple', './parseQuote', './parseSpaced', './Slice'], factory);
    }
    ...
    const parseObjSimple_1 = require('./parseObjSimple');
    ...
    function parseSingle(token) {
        ...
        if (token instanceof Token_1.NameToken) return new locals_1.LocalAccess(loc, token.name);else if (token instanceof Group_1.default) {
            ...
            } else if (token instanceof Group_1.GroupBracket) return new Val_1.BagSimple(loc, parseExpr_1.parseExprParts(Slice_1.Tokens.of(token)));else if (token instanceof Group_1.GroupBrace) return parseObjSimple_1.default(Slice_1.Tokens.of(token));else if (token instanceof Group_1.GroupBlock) return parseBlock_1.parseBlockWrap(Slice_1.Lines.of(token));else if (token instanceof Group_1.GroupQuote) return parseQuote_1.default(Slice_1.default.of(token));else if (token instanceof Group_1.GroupRegExp) return parseQuote_1.parseRegExp(Slice_1.default.of(token), token.flags);else throw new Error(String(token.type));
        } else if (token instanceof Token_1.NumberToken) return new Val_1.NumberLiteral(token.loc, token.value);else if (token instanceof Keyword_1.KeywordSpecialVal) return new Val_1.SpecialVal(loc, token.kind);else if (Keyword_1.isKeyword(46, token)) return locals_1.LocalAccess.focus(loc);else throw checks_1.unexpected(token);
    }
