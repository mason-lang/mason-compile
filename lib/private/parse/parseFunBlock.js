...
(function (factory) {
    ...
    function parsePolyValue(keywordKind, tokens) {
    ...
    const funFocusKeywords = new Set([93, 157, 163]);
    ...
    function tryTakeReturnType(tokens) {
        if (!tokens.isEmpty()) {
            const h = tokens.head();
            if (h instanceof Group_1.GroupSpace && Keyword_1.isKeyword(97, util_1.head(h.subTokens))) return [parseSpaced_1.default(Slice_1.Tokens.of(h).tail()), tokens.tail()];
        }
        return [null, tokens];
    }
