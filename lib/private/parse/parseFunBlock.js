...
(function (factory) {
    if (typeof module === 'object' && typeof module.exports === 'object') {
        ...
        define(["require", "exports", '../ast/Block', '../ast/Fun', '../ast/locals', '../ast/Method', '../token/Group', '../token/Keyword', '../util', './checks', './parseBlock', './parseCase', './parseLocalDeclares', './parseSpaced', './parseSwitch', './Slice', './tryTakeComment'], factory);
    }
    ...
    function parseMethodValue(keywordKind, tokens) {
        ...
        if (restLines.size() === 1) {
            ...
            if (h.size() === 1 && Keyword_1.isKeyword(77, h.head())) {
    ...
    function funArgsAndBlock(tokens, isVal) {
        ...
        if (Keyword_1.isAnyKeyword(funFocusKeywords, h)) {
            const expr = (h.kind === 86 ? parseCase_1.default : parseSwitch_1.default)(true, tokens.tail());
    ...
    const funFocusKeywords = new Set([86, 145]);
    ...
    function tryTakeReturnType(tokens) {
        if (!tokens.isEmpty()) {
            const h = tokens.head();
            if (h instanceof Group_1.GroupSpace && Keyword_1.isKeyword(90, util_1.head(h.subTokens))) return [parseSpaced_1.default(Slice_1.Tokens.of(h).tail()), tokens.tail()];
        }
        return [null, tokens];
    }
    function parseFunLocals(tokens) {
        ...
        if (tokens.isEmpty()) return { args: [], memberArgs: [], opRestArg: null };else {
            ...
            if (l instanceof Group_1.GroupSpace) {
                ...
                if (Keyword_1.isKeyword(97, g.head())) {
                    rest = tokens.rtail();
