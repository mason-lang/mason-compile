(function (factory) {
    ...
    function showToken(_, language) {
        if (_ instanceof Keyword_1.default) return code(_.name());else if (_ instanceof Group_1.default) return showGroup(_, language);else if (_ instanceof Token_1.DocComment) return 'doc comment';else if (_ instanceof Token_1.NameToken) return code(_.name);else if (_ instanceof Token_1.NumberToken || _ instanceof Token_1.StringToken) return _.value;else throw new Error(_.constructor.name);
    }
