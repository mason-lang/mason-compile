...
(function (factory) {
    if (typeof module === 'object' && typeof module.exports === 'object') {
        ...
        define(["require", "exports", '../MsAst', '../Token', '../util', './checks', './parseBlock', './parseCase', './parseLocalDeclares', './parseSpaced', './parseSwitch', './Slice', './tryTakeComment'], factory);
    }
    ...
    function parseFun(keywordKind, tokens) {
        ...
        return new MsAst_1.Fun(tokens.loc, args, opRestArg, block, { kind: kind, isThisFun: isThisFun, isDo: isDo, opReturnType: opReturnType });
    }
    ...
    function parseFunLike(keywordKind, tokens) {
        ...
        if (restLines.size() === 1) {
            ...
            if (h.size() === 1 && Token_1.isKeyword(34, h.head())) {
                var _parseFunLocals = parseFunLocals(before);

                const args = _parseFunLocals.args;
                const opRestArg = _parseFunLocals.opRestArg;

                return new MsAst_1.FunAbstract(tokens.loc, args, opRestArg, opReturnType, opComment);
            }
        }
    ...
    function funArgsAndBlock(tokens, isVal) {
        ...
        const h = tokens.head();
        if (Token_1.isAnyKeyword(funFocusKeywords, h)) {
            const expr = (h.kind === 43 ? parseCase_1.default : parseSwitch_1.default)(true, tokens.tail());
            const args = [MsAst_1.LocalDeclare.focus(h.loc)];
            return { args: args, opRestArg: null, memberArgs: [], block: new MsAst_1.Block(tokens.loc, null, [expr]) };
        } else {
    ...
    const funFocusKeywords = new Set([43, 101]);
    ...
    function tryTakeReturnType(tokens) {
        if (!tokens.isEmpty()) {
            const h = tokens.head();
            if (h instanceof Token_1.GroupSpace && Token_1.isKeyword(47, util_1.head(h.subTokens))) return [parseSpaced_1.default(Slice_1.Tokens.of(h).tail()), tokens.tail()];
        }
        return [null, tokens];
    }
    function parseFunLocals(tokens) {
        ...
        if (tokens.isEmpty()) return { args: [], memberArgs: [], opRestArg: null };else {
            let rest = tokens,
                opRestArg = null;
            const l = tokens.last();
            if (l instanceof Token_1.GroupSpace) {
                const g = Slice_1.Tokens.of(l);
                if (Token_1.isKeyword(54, g.head())) {
                    rest = tokens.rtail();
