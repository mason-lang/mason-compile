(function (factory) {
    if (typeof module === 'object' && typeof module.exports === 'object') {
        ...
        define(["require", "exports", 'op/Op', '../context', '../MsAst', '../Token', './checks', './parseBlock', './parseExpr', './parseQuote', './parseSpaced', './Slice'], factory);
    }
    ...
    function parseSingle(token) {
        ...
        if (token instanceof Token_1.Name) return new MsAst_1.LocalAccess(loc, token.name);else if (token instanceof Token_1.Group) {
            if (token instanceof Token_1.GroupSpace) return parseSpaced_1.default(Slice_1.Tokens.of(token));else if (token instanceof Token_1.GroupParenthesis) {
            ...
            } else if (token instanceof Token_1.GroupBracket) return new MsAst_1.BagSimple(loc, parseExpr_1.parseExprParts(Slice_1.Tokens.of(token)));else if (token instanceof Token_1.GroupBlock) return parseBlock_1.parseBlockWrap(Slice_1.Lines.of(token));else if (token instanceof Token_1.GroupQuote) return parseQuote_1.default(Slice_1.default.of(token));else if (token instanceof Token_1.GroupRegExp) return parseQuote_1.parseRegExp(Slice_1.default.of(token), token.flags);else throw new Error(String(token.type));
