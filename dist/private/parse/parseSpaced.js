(function (factory) {
    ...
    function parseSpaced(tokens) {
        ...
        if (h instanceof Keyword_1.default) switch (h.kind) {
            case 77:
                {
                    const h2 = rest.head();
                    if (h2 instanceof Group_1.GroupParenthesis) return new Fun_1.SimpleFun(tokens.loc, parseExpr_1.default(Slice_1.Tokens.of(h2)));else if (Keyword_1.isKeyword(94, h2)) {
    ...
    function parseSpacedFold(start, rest) {
        ...
        for (let i = rest.start; i < rest.end; i = i + 1) {
            ...
            } else if (token instanceof Group_1.GroupQuote) acc = new Val_1.QuoteTaggedTemplate(loc, acc, parseQuote_1.default(Slice_1.default.of(token)));else throw checks_1.unexpected(token);
        }
        return acc;
    }
});
